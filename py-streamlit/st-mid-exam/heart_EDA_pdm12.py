...
st.sidebar.subheader('Visualization using seaborn and plotly by pdm12')
...
with row2_2: 
    corr_y = st.selectbox("Correlation - Y variable", options=df.columns, index=df.columns.get_loc("trestbps"))
...
df_strong = df[['age', 'cp', 'thalach', 'exang', 'oldpeak', 'slope', 'ca', 'thal']]
sns.heatmap(df_strong.corr(),annot=True, vmin=-1, vmax=1, cmap='coolwarm')
st.pyplot(fig6)
